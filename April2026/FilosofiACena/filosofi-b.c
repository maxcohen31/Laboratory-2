...
static atomic_long ITER = 2000;  // durata della cena
...
void Lock(forchetta_t* destra, forchetta_t* sinistra) {
  do {
    LOCK(destra);
    if (TRYLOCK(sinistra) == 0) return;  // ho entrambe le lock
    UNLOCK(destra);

    // provo nell'ordine inverso
    LOCK(sinistra);                     
    if (TRYLOCK(destra) == 0) return;   // ho entrambe le lock
    UNLOCK(sinistra);    
  } while(1);
}
void UnLock(forchetta_t* destra, forchetta_t* sinistra) {
  UNLOCK(destra);
  UNLOCK(sinistra);
}
...
void *Filosofo(void *arg) {
	...
	Lock(destra, sinistra);
	++mangia;
	Mangia(myid, &seed);
	UnLock(destra, sinistra);

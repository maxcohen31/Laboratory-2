...
/**
 * @function isRegular
 * @brief Check if the name provided is a directory.
 *        It returns 1 if success, 0 if it is not a directory, and -1 if error.
 */
static inline int isDirectory(const char name[]) {
  struct stat statbuf;
  if (stat(name, &statbuf) == -1) return -1;
